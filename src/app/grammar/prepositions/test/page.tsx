...
const questions = grammarPrepositionsQuiz;

export default function PrepositionsTestPage() {
  ...
  if (scored) {
    return (
      <div className="app-panel">
        <QuizResultsSummary
          resultsHeading="Prepositions Test Results"
          score={score}
          totalQuestions={shuffledQuestions.length}
          quizPath={pathname}
          backHref="/grammar/prepositions"
  ...
  return (
    <div className="app-panel">
      <h1 className="mb-4 text-center text-4xl font-bold text-black dark:text-zinc-50">Prepositions Test</h1>
      <div className="mb-4">
      ...
      <TestQuestionNavLayout
        ...
        renderRight={
          currentQuestion < shuffledQuestions.length - 1 ? <TestNextQuestionButton onClick={nextQuestion} /> : undefined
        }
        scoreFooterAction={currentQuestion === shuffledQuestions.length - 1 ? calculateScore : undefined}
      >
        <div>
          ...
          <div className="space-y-2">
            {currentQ.options.map((option, oIndex) => (
              <label
                key={oIndex}
                className={`block cursor-pointer rounded border p-3 ${getOptionClass(oIndex)} ${answers[currentQuestion] === oIndex ? 'border-blue-500' : 'border-gray-300 dark:border-gray-600'}`}
              >
                <input
                  type="radio"
                  name="answer"
